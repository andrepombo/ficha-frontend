...
class AuthService {
  ...
  async login(username: string, password: string): Promise<AuthResponse> {
    const isDemoAttempt = password === DEMO_PASSWORD;

    const attempt = async (user: string, pass: string, visitorEmail?: string) => {
      return axios.post<AuthResponse>(`${API_BASE_URL}/token/`, {
        username: user,
        password: pass,
    ...
    try {
      if (isDemoAttempt) {
        // Always use configured demo credentials for demo attempts
        response = await attempt(DEMO_USERNAME, DEMO_PASSWORD, username);
      } else {
        response = await attempt(username, password);
      }
    } catch (error: any) {
      throw error;
    }
    ...
    if (user) {
      // Preserve the visitor email for demo mode tracking
      const mergedUser = { ...user };
      if (isDemoAttempt && username) {
        mergedUser.email = username;
      }
