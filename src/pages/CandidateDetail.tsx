  ...
  return (
    <div className="bg-purple-50">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {/* Tabs below header - Modern pill style */}
        <div className="mt-6 mb-8">
          <div className="flex items-center gap-3 overflow-x-auto pb-2 px-2">
            <button
              onClick={() => setActiveTab('personal')}
              className={`group relative px-6 py-3 rounded-full font-semibold text-sm transition-all duration-300 whitespace-nowrap flex-shrink-0 ${
                activeTab === 'personal'
                  ? 'bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 text-white shadow-lg shadow-purple-500/50 scale-105'
                  : 'bg-white text-gray-700 hover:bg-gradient-to-r hover:from-indigo-50 hover:to-purple-50 hover:text-indigo-700 shadow-md hover:shadow-lg border border-purple-100'
              }`}
            >
              ...
              >
                <span className="flex items-center gap-2">
                  ...
                  <span className={`ml-1 px-2 py-0.5 rounded-full text-xs font-bold ${
                    ...
                    {Number(r.score || 0).toFixed(0)}/{Number(r.max_score || 0)}
                  </span>
                </span>
                {activeTab === `q-${r.id}` && (
                  <span className="absolute -bottom-1 left-1/2 transform -translate-x-1/2 w-2 h-2 bg-white rounded-full"></span>
              ...
              <div className="space-y-6">
                {questions.length > 0 ? (
                  questions.map((question: any, idx: number) => {
                    ...
                    // Determine weighted scoring color based on selected answer quality
